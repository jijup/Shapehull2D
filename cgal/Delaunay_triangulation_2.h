...
//
// $URL: svn+ssh://scm.gforge.inria.fr/svn/cgal/branches/releases/CGAL-3.9-branch/Triangulation_2/include/CGAL/Delaunay_triangulation_2.h $
// $Id: Delaunay_triangulation_2.h 63930 2011-06-07 08:20:54Z sloriot $
// 
//
...
typedef typename Geom_traits::Compute_area_2 Area;
...
// CHECK -QUERY
 ...
 void delete_boundary_faces(Face_handle fh, int i);
 void delete_boundary_faces1(Face_handle fh, int i,int ind1,int ind2);
 void delete_interior_face(Face_handle fh);
 bool junction_point(Face_handle fh, int i);
 bool dangling_edge(Face_handle fh);
 bool is_boundary_face(Face_handle fh);
 bool is_equal(Face_handle f1, Face_handle f2);
 bool is_thin_face(Face_handle fh);
 bool is_longest_edge_shared(Face_handle fh1,Face_handle fh2,int index);
 bool is_two_edge_shared(Face_handle fh);
 bool is_one_edge_shared(Face_handle fh);
 void set_circum_radius(Face_handle f,double r);
 ...
 //HOLE DETECTION
//void create_hole(Face_handle fh);
...
//Methods for shape-hull
template < class Gt, class Tds >
void
Delaunay_triangulation_2<Gt,Tds>::
delete_boundary_faces(Face_handle fh, int index) 
{
int rindex,lindex,ind;

Vertex_handle v=fh->vertex(index);

Face_handle fr=fh->neighbor(ccw(index));
rindex=this->mirror_index(fh,ccw(index));
Face_handle fl=fh->neighbor(cw(index));
lindex=this->mirror_index(fh,cw(index));
ind=this->mirror_index(fh,index);

/*std::cout<<"\nDeleting face\n"<<this->circumcenter(fh);
Face_handle candidate=fh->neighbor(index);
Face_handle  neighborr=candidate->neighbor(cw(ind));
Face_handle  neighborl=candidate->neighbor(ccw(ind));
int indr=this->mirror_index(candidate,cw(ind));
int indl=this->mirror_index(candidate,ccw(ind));*/

this->tds().delete_face(fh->neighbor(index));
this->tds().delete_face(fh);
Face_handle newf1=this->tds().create_face(fr,rindex,this->infinite_vertex());
Face_handle newf2=this->tds().create_face(fl,lindex,this->infinite_vertex());

/*
newf1->set_neighbor(cw(rindex),newf2);
newf2->set_neighbor(ccw(lindex),newf1);
newf1->set_neighbor(ccw(rindex),neighborr);
newf2->set_neighbor(cw(lindex),neighborl);
neighborr->set_neighbor(indr,newf2);
neighborl->set_neighbor(indl,newf1);*/
...
void
Delaunay_triangulation_2<Gt,Tds>::
delete_boundary_faces1(Face_handle fh, int index,int ind1,int ind2) 
{

this->tds().delete_face(fh->neighbor(index)->neighbor(ind1));
this->tds().delete_face(fh->neighbor(index)->neighbor(ind2));
this->tds().delete_face(fh->neighbor(index));
Face_handle newf1=this->tds().create_face(fh,index,this->infinite_vertex());
}

template < class Gt, class Tds >
void
Delaunay_triangulation_2<Gt,Tds>::
delete_interior_face(Face_handle fh) 
{
int index,rindex,lindex;

//Vertex_handle v=fh->vertex(index);

Face_handle f1=fh->neighbor(0);
index=this->mirror_index(fh,0);
Face_handle f2=fh->neighbor(1);
lindex=this->mirror_index(fh,1);
Face_handle f3=fh->neighbor(2);
rindex=this->mirror_index(fh,2);

std::cout<<"\nDeleting face\n"<<this->circumcenter(fh);
//this->tds().delete_face(fh->neighbor(index));
this->tds().delete_face(fh);
Face_handle newf1=this->tds().create_face(f2,lindex,this->infinite_vertex());
Face_handle newf2=this->tds().create_face(f3,rindex,this->infinite_vertex());
Face_handle newf3=this->tds().create_face(f1,index,this->infinite_vertex());
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
junction_point(Face_handle fh, int i) 
{
...
do{
	...
	std::cout<<"\n Hello"<<circumcenter(fc);
}while(++fc!=done);

if(ct)
{
	std::cout<<"Its a junction point creator" <<std::endl;
	return 1;
}
else
return 0; 
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
dangling_edge(Face_handle fh)
{
int flag=0;
for(int i=0;i<3;i++)
{
if(this->is_infinite(fh->neighbor(i)))
++flag;
}
if(flag>1)
{
std::cout<<"its a dangling edge creator"<<std::endl;
return 1;
}
else
return 0;
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
is_equal(Face_handle f1, Face_handle f2)
{
if(f1==f2)
return 1;
else
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
is_thin_face(Face_handle fh)
{
for(int i=0;i<3;i++)
{
if(!this->is_equal(this->locate(this->circumcenter(fh),fh),fh))
return 1;
}
return 0;
}
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
is_boundary_face(Face_handle fh)
{
for(int i=0;i<3;i++)
	{
		if(this->is_infinite(fh->neighbor(i)))
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
is_longest_edge_shared(Face_handle fh1,Face_handle fh2,int index)
{
	FT max_length=FT(0);
	FT length=FT(0);
	//int ind1,ind2;
	Segment s1=Segment(fh1->vertex(this->cw(index))->point(),fh1->vertex(this->ccw(index))->point());
	max_length=s1.squared_length();
	index=this->cw(index);
	Segment s2=Segment(fh1->vertex(this->cw(index))->point(),fh1->vertex(this->ccw(index))->point());
	length=s2.squared_length();
	std::cout<<"length"<<length;
	if(length>max_length)
	return(0);
	index=this->cw(index);
	Segment s3=Segment(fh1->vertex(this->cw(index))->point(),fh1->vertex(this->ccw(index))->point());
	length=s3.squared_length();
	if(length>max_length)
	return(0);
	return(1);
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
is_two_edge_shared(Face_handle fh)
{
int count=0;
for(int i=0;i<3;i++)
	{
		int index=this->mirror_index(fh,i);
		if(this->is_thin_face(fh->neighbor(i))&&this->is_longest_edge_shared(fh->neighbor(i),fh,index))
			count++;
	}
if(count==2)
return 1;
...
inline bool
Delaunay_triangulation_2<Gt,Tds>::
is_one_edge_shared(Face_handle fh)
{
int count=0;
for(int i=0;i<3;i++)
	{
...
void
Delaunay_triangulation_2<Gt,Tds>::
set_circum_radius(Face_handle f,double r)
{
	this->tds().set_cradius(f,r);
}
